    ...
    private async ValueTask<IMongoCollection<Mint>> Get()
    {
        if (_collection is null)
        {
            ...
            await _collection.Indexes.CreateManyAsync([
                new CreateIndexModel<Mint>(Builders<Mint>.IndexKeys.Ascending(x => x.ContractId).Ascending(x => x.ContentId)),
                new CreateIndexModel<Mint>(Builders<Mint>.IndexKeys.Ascending(x => x.ContractId).Ascending(x => x.ContentId).Ascending(x => x.InitialOwnerAddress))
            ]);
        }
