    ...
    : IService
{
    ...
    {
        var binaryResponse = await contentApi.PostBinary(new SaveBinaryRequest
        {
            binary =
            [
    ...
    private async Task<string> StoreExternalMetadata(string metadataJson)
    {
        ...
        var saveBinaryResponse = await TaskExtensions.WithRetry(
            async () => await contentApi.PostBinary(new SaveBinaryRequest
            {
                binary =
                [
                    new BinaryDefinition
    ...
    public async Task<NftExternalMetadata> BuildMetadata(uint tokenId, string contentId, Dictionary<string, string> requestProperties)
    {
        var metadata = new NftExternalMetadata();
        var contentDefinition = await contentService.GetContent(contentId);
        metadata.Load(contentDefinition, tokenId, requestProperties);
        metadata.ExternalUrl = await configuration.NftExternalUrl;
        return metadata;
    }
}