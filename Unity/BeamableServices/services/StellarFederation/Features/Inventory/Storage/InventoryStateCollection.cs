...
namespace Beamable.StellarFederation.Features.Inventory.Storage;
...
public class InventoryStateCollection : IService
{
    ...
    private async ValueTask<IMongoCollection<InventoryState>> Get()
    {
        if (_collection is null)
            _collection = (await _storageObjectConnectionProvider.StellarFederationStorageDatabase()).GetCollection<InventoryState>("inventory-state");
