...
namespace Beamable.StellarFederation.Features.Notifications;
...
public class PlayerNotificationService : IService
{
    ...
    public async Task SendPlayerNotification(IEnumerable<long> gamerTags, IPlayerNotification payload)
    {
        var targets = gamerTags.ToList();
        try
        {
            var message = JsonConvert.SerializeObject(payload, _settings);
            foreach (var gamerTag in targets)
            {
                var request = new
                {
                    dbid = gamerTag,
        ...
        {
            BeamableLogger.LogWarning($"Unable to send players {string.Join(", ", targets)} notification.");
        }
    }
}