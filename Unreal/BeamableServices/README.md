...
Read more on the [Content System](https://help.beamable.com/Unreal-Latest/unreal/user-reference/beamable-services/content/)
...
In Unreal Engine, content is managed through the Beamable CLI and represented as DataTables or native Unreal data assets:

**Using Beamable CLI:**

1. **Initialize Content Workspace** (one-time setup):
   ```bash
...
2. **Create Content Manifest**:
   ...
   Content definitions are stored as JSON in `.beamable/content/<PROJECT_ID>/global/`:

   ```json
   {
     "id": "items.weapon.sword",
     "type": "items.weapon",
     "properties": {
       "name": "Legendary Sword",
       "description": "A powerful blade forged in ancient fires",
...
3. **Publish Content**:
   ```bash
   # Publish to your current realm
   dotnet beam content publish

   # Publish to specific realm
   dotnet beam content publish --realm dev
   ```

4. **Verify Published Content**:
   ```bash
   dotnet beam content list
   ```

**Using Unreal Editor:**

Beamable provides content manipulation directly in the editor:

1. Open **Tools > Beamable > Content Manager**
2. Select **Create New Content**
3. Choose your content type (e.g., "items.weapon")
4. Fill in properties using the property editor
5. Click **Publish** to sync with Beamable backend

**Content DataTables:**

Content can also be represented as Unreal DataTables for designer-friendly editing:
...
Create a DataTable asset in the editor:
- Right-click in Content Browser > **Miscellaneous > Data Table**
- Select `FWeaponItemData` as the row structure
- Add rows for each weapon variant
- Export to Beamable content format via CLI

**Content Workflow:**

```
Designer Creates Content → DataTable/JSON → CLI Publish →
Beamable Backend → Runtime Download → Unreal Client
```

**Blueprint Access:**

Access content at runtime using Beamable's Blueprint nodes:

```
GetBeamableContent → Filter by Content Type →
Get Content by ID → Read Properties
```

Or in C++:

```cpp
// Get content subsystem
UBeamableContentSubsystem* ContentSystem = GetGameInstance()->GetSubsystem<UBeamableContentSubsystem>();

// Load content
ContentSystem->GetContent("items.weapon.sword", [](FBeamableContent Content)
{
    FString WeaponName = Content.GetProperty("name");
    int32 Damage = FCString::Atoi(*Content.GetProperty("damage"));
    UE_LOG(LogTemp, Log, TEXT("Loaded weapon: %s with damage: %d"), *WeaponName, Damage);
});
```

**Content Sync:**

Content automatically syncs when the game starts:
- Downloads latest content from Beamable on launch
- Caches locally for offline access
- Validates content integrity
- Notifies game of updates via delegates/events
...
dotnet beam content pull

# View content status
dotnet beam content status

# Validate content before publishing
dotnet beam content validate

# Publish specific content type
dotnet beam content publish --type items.weapon
```
...
1. **Development**: Create/edit content in Content Manager
...
1. Create test content in Content Manager
...
- [Beamable SDK 3.0 Content System](https://help.beamable.com/Unreal-Latest/unreal/user-reference/beamable-services/content/)
...
- [Beamable Documentation](https://help.beamable.com/Unreal-Latest/)
